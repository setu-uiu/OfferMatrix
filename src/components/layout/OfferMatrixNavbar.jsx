...
import {
  ...
  Home, UtensilsCrossed, Plane, Sparkles, Tag, RotateCcw, Heart,
} from 'lucide-react'
      ...
      <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
        ...
        <form onSubmit={handleSearch} className="hidden md:flex flex-1 max-w-md relative">
          ...
          <button type="submit" aria-label="Search" className="absolute right-3 top-1/2 -translate-y-1/2">
            <Search size={16} className="text-blue-200 hover:text-white transition" />
          </button>
        </form>

        {/* Right actions */}
        <div className="flex items-center gap-3">
          <Link to="/account" aria-label="Account" className="hidden md:flex items-center gap-1.5 text-blue-100 hover:text-white text-sm transition">
            <User size={18} />
            <span className="hidden lg:inline">Login</span>
          </Link>
          {/* Wishlist */}
          <Link to="/wishlist" aria-label="Wishlist" className="hidden md:flex relative items-center text-blue-100 hover:text-white transition">
            <Heart size={18} />
            {wishCount > 0 && (
              <span className="absolute -top-1.5 -right-1.5 w-4 h-4 flex items-center justify-center bg-pink-500 text-white text-[9px] font-bold rounded-full">
                {wishCount}
              </span>
            )}
          </Link>
          {/* Cart */}
          <Link to="/cart" aria-label="Cart" className="hidden md:flex relative items-center gap-1.5 bg-cyan-500 hover:bg-cyan-400 text-white text-xs font-semibold px-3 py-1.5 rounded-full transition">
            <ShoppingCart size={13} />
            Cart
            {cartCount > 0 && (
              <span className="bg-white text-cyan-600 text-[9px] font-bold px-1.5 py-0.5 rounded-full leading-none">
                {cartCount}
              </span>
            )}
          </Link>
          ...
          <div className="relative mb-3">
            <form onSubmit={(e) => {
              ...
              <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2">
                <Search size={15} className="text-blue-200" />
              </button>
            </form>
          </div>
