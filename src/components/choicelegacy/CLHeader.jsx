...
const mobileNavItems = [
  { label: 'Home',             to: '/choice-legacy' },
  { label: 'Skincare',         to: '/search?q=Skincare' },
  { label: 'Haircare',         to: '/search?q=Haircare' },
  { label: 'Body Care',        to: '/search?q=Body+Care' },
  { label: 'Makeup',           to: '/search?q=Makeup' },
  { label: 'Intimate & Hygiene', to: '/search?q=Hygiene' },
  { label: 'Beauty Tools',     to: '/search?q=Beauty+Tools' },
    ...
    <header className="bg-white" style={{ borderBottom: `1px solid ${CL.border}` }}>
        ...
        <div className="max-w-7xl mx-auto px-4 h-8 flex items-center justify-between text-xs text-gray-500">
          <div className="flex items-center gap-5">
            <span className="flex items-center gap-1.5">
              <Phone size={11} className="text-gray-400" /> +880 1800-000000
            </span>
            <span className="flex items-center gap-1.5">
              <MapPin size={11} className="text-gray-400" /> Dhaka, Bangladesh
            </span>
          </div>
          <div className="flex items-center gap-5">
            <span style={{ color: CL.gold }} className="font-medium">Free delivery above ৳1,200</span>
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center gap-4 h-16 md:h-20">
          ...
          <Link to="/choice-legacy" className="flex items-center gap-2.5 shrink-0">
            ...
            <div className="hidden sm:block leading-none">
              <span
                ...
                style={{ fontSize: '1.2rem', color: CL.primary, fontFamily: '"Playfair Display", Georgia, serif', letterSpacing: '0.02em' }}
              >
                Choice
              </span>
              <span
          ...
          {/* Search — desktop */}
          <form onSubmit={handleSearch} className="hidden md:flex flex-1 min-w-0 max-w-xl mx-4 relative">
            <input
              type="search"
              value={query}
              ...
              style={{ border: `1.5px solid ${CL.border}`, borderRadius: '8px' }}
            ...
            <button
              ...
              className="absolute right-1 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center
                justify-center rounded-lg transition text-white"
          ...
          <div className="md:hidden pb-3">
            <form onSubmit={handleSearch} className="relative">
              <input
                ...
                style={{ border: `1.5px solid ${CL.gold}`, borderRadius: '8px', color: CL.primary }}
              />
              <button
                type="submit"
                aria-label="Search"
                className="absolute right-1 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center
                  justify-center rounded-lg text-white"
                style={{ backgroundColor: CL.primary }}
              >
                <Search size={15} />
              </button>
