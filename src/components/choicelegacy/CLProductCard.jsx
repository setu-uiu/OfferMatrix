...
export default function CLProductCard({ product }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {badge && (
          <span
            className="absolute top-2.5 right-2.5 text-white text-[10px] font-bold px-2 py-0.5 rounded-full"
        ...
        >
          <Heart
            size={14}
            style={wished ? { fill: CL.accent, color: CL.accent } : { color: '#9CA3AF' }}
          />
        </button>
      ...
      <div className="flex flex-col flex-1 p-3.5 gap-1.5">
        ...
        {/* Name */}
        <p className="text-sm font-medium leading-snug line-clamp-2" style={{ color: CL.primary }}>
          {name}
        </p>
        ...
        >
          ...
          {!inStock ? 'Out of Stock' : (added || inCart) ? 'In Cart ✓' : 'Add to Cart'}
        </button>
      </div>
    </div>
  )
