...
export default function KireiHeader() {
  ...
  const mobileNavItems = [
    { label: 'Home',                to: '/kirei' },
    { label: 'J-Beauty',            to: '/search?q=J-Beauty' },
    { label: 'K-Beauty',            to: '/search?q=K-Beauty' },
    { label: 'International Brands',to: '/search?q=International' },
    { label: 'Offers',              to: '/coupons' },
    { label: 'Baby Care',           to: '/search?q=Baby+Care' },
    { label: 'Combo',               to: '/search?q=Combo' },
    { label: 'Makeup',              to: '/search?q=Makeup' },
    { label: 'Doctor Cosmetics',    to: '/search?q=Doctor+Cosmetics' },
    { label: 'Travel Kit',          to: '/search?q=travel+kit' },
  ]
      ...
      <div className="hidden md:block bg-[#FDF8F5] border-b border-[#F5D6E0]">
        <div className="max-w-7xl mx-auto px-4 h-8 flex items-center justify-between text-xs text-gray-500">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1"><Phone size={11} /> +880 1700-000000</span>
            <span className="flex items-center gap-1"><MapPin size={11} /> Dhaka, Bangladesh</span>
          </div>
          <div className="flex items-center gap-4">
            <span>Free shipping on orders above ৳1,500</span>
            <Link to="/search?q=J-Beauty" className="text-[#D4527A] font-medium hover:underline">
              J-Beauty Users Group →
            </Link>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center gap-4 h-16 md:h-20">
          ...
          <form onSubmit={handleSearch} className="hidden md:flex flex-1 max-w-xl relative mx-4">
            <input
              ...
              className="w-full pl-5 pr-12 py-2.5 rounded-full border-2 border-[#F5D6E0] bg-[#FDF8F5] text-sm
                text-gray-700 placeholder-gray-400 focus:outline-none focus:border-[#D4527A] transition"
            />
            <button
              type="submit"
              aria-label="Search"
          ...
          <div className="flex items-center gap-1 sm:gap-2 ml-auto">
            ...
            <Link
              to="/account"
              className="hidden sm:flex items-center gap-1.5 ml-1 px-3 py-1.5 rounded-full border-2 border-[#E8A4B8]
            ...
            {mobileNavItems.map(item => (
              <Link
                ...
                className="px-3 py-2.5 rounded-lg text-sm font-medium text-gray-700
                  hover:bg-[#FDF0F4] hover:text-[#D4527A] transition"
              >
                {item.label}
              </Link>
            ))}
